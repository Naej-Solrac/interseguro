...
import { getMatricesStatistics } from '../../../application/usecases/get-matrices-statistics.usecase';
 ...
 *                 summary:
 *                   $ref: '#/components/schemas/MatrixStatistics'
 *                 hasAnyDiagonalMatrix:
 *                   type: boolean
 *                   description: Indica si al menos una de las matrices es diagonal
 *             example:
 *               Q:
 *                 avg: 0.392
 *                 min: -0.345
 ...
 */
export async function postMatrixStatsController(req: Request, res: Response): Promise<void> {
  try {
    // Ya está validado por Joi, podemos confiar en los datos
    
    const { Q: qMatrix, R: rMatrix } = req.body;
    
    const matrixStatistics = getMatricesStatistics(qMatrix, rMatrix);
    res.status(200).json(matrixStatistics);
  } catch (err: any) {
